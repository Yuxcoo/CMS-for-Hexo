  ...
  return (
    <div className="grid gap-4">
      <section className="apple-card flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="font-display text-[22px] font-semibold leading-[1.14] tracking-[-0.2px] text-ink">图片库</h2>
          <p className="mt-2 text-[15px] leading-[1.45] tracking-[-0.18px] text-muted">上传后可直接复制 Markdown 引用。</p>
        </div>
        <label className="inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-full bg-blue px-4 py-2 text-[15px] font-normal leading-none tracking-[-0.18px] text-white transition hover:bg-blueFocus active:scale-95">
          <Upload size={17} /> 上传图片
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {items.map((item) => (
          <article key={item.path} className="overflow-hidden rounded-[14px] border border-line bg-canvas">
            <div className="grid aspect-square place-items-center bg-paper p-4">
            ...
            <div className="grid gap-2 p-4">
              <div className="truncate text-[15px] font-semibold leading-[1.24] tracking-[-0.18px] text-ink">{item.name}</div>
